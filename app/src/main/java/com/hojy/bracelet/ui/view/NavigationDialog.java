...
public class NavigationDialog extends AlertDialog {
    ...
    RelativeLayout mLlBaiduMap;

    @BindView(R.id.ll_gaode_map)
    RelativeLayout mLlGaodeMap;
    ...
    public NavigationDialog(Context context, MapActivity devicesHomeActivity) {
        super(context, R.style.MyDialog);
        mContext = context;
        mMapActivity = devicesHomeActivity;
    ...
    public void onViewClicked(View view) {
        String address = (String) mMapActivity.mTvAddress.getText();
        double lat = mMapActivity.mLat;
        double lng = mMapActivity.mLng;
        switch (view.getId()) {
            case R.id.tv_baidu:
    ...
    private void navigationBaiduMap(double lat, double lng) {
        try {
            if (lat == 0) {
                mMapActivity.printn(mContext.getString(R.string.don__get_to_watch));
                dismiss();
                return;
            }
            ...
            if (isInstallByread("com.baidu.BaiduMap")) {
                ...
                mMapActivity.startActivity(i1);
                dismiss();
            } else {
                mMapActivity.printn(mContext.getString(R.string.install_baidu_map));
                dismiss();
            }
        } catch (Exception e) {
    ...
    private void navigationGaoDeMap(double lat, double lon) {
        ...
        try {
            if (isInstallByread("com.autonavi.minimap")) {
                ...
                intent1.setData(uri);
                //启动该页面即可
                mMapActivity.startActivity(intent1);
                dismiss();
            } else {
                mMapActivity.printn(mContext.getString(R.string.install_gaode));
                dismiss();
            }
        } catch (Exception e) {
            LogUtils.e(TAG, e.getMessage());
